"""
    ...
    36th Conference on Neural Information Processing Systems (ACMMM 2023)
...
parser.add_argument('--gamma', default=0.2, type=float, help='mask ratio')
parser.add_argument('--alpha', default=1, type=float, help='mask loss ratio')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = feasc.builder.BYOL_MF(
        feasc.resnet.resnet50,
    ...
    augmentation1 = [
        ...
        transforms.RandomApply([feasc.loader.GaussianBlur([.1, 2.])], p=1.0),
    ...
    augmentation2 = [
        ...
        transforms.RandomGrayscale(p=0.2),
        transforms.RandomApply([feasc.loader.GaussianBlur([.1, 2.])], p=0.1),
        transforms.RandomApply([feasc.loader.Solarize()], p=0.2),
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        normalize
    ]
    ...
    train_dataset = datasets.ImageFolder(
        traindir,
        feasc.loader.TwoCropsTransform(transforms.Compose(augmentation1), 
...
def train(train_loader, model, optimizer, epoch, lr_schedule, momentum_schedule, lamda_schedule, args):
    ...
    losses_54 = AverageMeter('L_54', ':.4f')
    losses_53 = AverageMeter('L_53', ':.4f')
    
    losses_81 = AverageMeter('L_81', ':.4f')
    losses_82 = AverageMeter('L_82', ':.4f')

    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, losses_54, losses_53, losses_81, losses_82, losses],
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        gamma = get_current_consistency_weight(epoch, args.gamma, 20)

        # compute output and loss
        z1, z2, z1_l, z2_l, p1, p2 = model(x1=images[0], x2=images[1], gamma=gamma)

        loss_54 = loss_byol(z1, p2)
        loss_53 = loss_byol(z1_l, p2)
        
        loss_81 = loss_byol(z2, p1)
        loss_82 = loss_byol(z2_l, p1)

        loss = torch.mean(loss_54 + loss_81) / 2 + args.alpha * torch.mean(loss_53 + loss_82) / 2

        losses_54.update(torch.mean(loss_54), images[0].size(0))
        losses_53.update(torch.mean(loss_53), images[0].size(0))
        
        losses_81.update(torch.mean(loss_81), images[0].size(0))
        losses_82.update(torch.mean(loss_82), images[0].size(0))
